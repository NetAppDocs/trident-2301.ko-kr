...
SVM에 전용 또는 공유 관리 LIF를 사용하는 것이 더 이상 선호되지 않지만, 선택한 접근 방식에 맞게 네트워크 보안 정책을 조정해야 합니다. 이와 관계없이 Trident과 함께 사용하면 DNS를 통해 관리 LIF에 액세스할 수 있어야 최대한의 유연성을 발휘할 수 https://docs.netapp.com/us-en/ontap/data-protection/snapmirror-svm-replication-concept.html["SVM-DR"^] 있습니다.
...
SVM에 적용되는 ONTAP QoS 정책을 활용하여 Trident에서 프로비저닝된 볼륨에서 사용할 수 있는 IOPS 수를 제한합니다. 그러면 https://docs.netapp.com/us-en/ontap/performance-admin/guarantee-throughput-qos-task.html["괴롭힘을 방지합니다"^] Trident SVM 외부에서 워크로드에 영향을 미치는 컨테이너가 제어 불능 상태가 됩니다.
...
QoS 정책 그룹에 대한 자세한 내용은 를 https://docs.netapp.com/us-en/ontap-cli-98/index.html["ONTAP 9.8 QoS 명령"^]참조하십시오.
...
* https://docs.netapp.com/us-en/ontap/san-admin/index.html["SAN 스토리지 관리"^] (iSCSI용)
* https://docs.netapp.com/us-en/ontap/san-admin/provision-storage.html["RHEL용 iSCSI Express 구성"^]
